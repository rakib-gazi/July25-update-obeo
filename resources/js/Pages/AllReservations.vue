                ...
                <template #item-status_id="item">
                    <div class="text-sm font-semibold">
                        ...
                        <select
                            ...
                            :class="{
                                'text-green-500': item.status === 'Checked In',
                                'text-yellow-500': item.status === 'No show',
                                'text-red-500': item.status === 'Canceled',
                                'text-gray-500': item.status !== 'Checked In' && item.status !== 'No show' && item.status !== 'Canceled'
                              }"
                            class="bg-gray-50  border-gray-300  text-sm rounded-lg border-0 bg-transparent  focus:border-0 block w-full p-2.5 "
                        >
                            ...
                            <option
                                ...
                                :value="status.id"
                                :class="{
                                'text-green-500': status.status === 'Checked In',
                                'text-yellow-500': status.status === 'No show',
                                'text-red-500': status.status === 'Canceled',
                                'text-gray-500': status.status !== 'Checked In' && status.status !== 'No show' && status.status !== 'Canceled'
                              }"
                            >
                                {{ status.status }}
                            </option>
